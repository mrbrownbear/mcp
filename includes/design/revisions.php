...
namespace Novamira\Design\Revisions;
...
function restore(WP_Post $post, WP_Post $revision, string $actor = 'user'): int|WP_Error
{
    if ((int) $revision->post_parent !== $post->ID) {
        return new WP_Error('revision_foreign', __(
            'That revision does not belong to this design.',
            domain: 'novamira',
        ));
    }
    ...
    if (Store\get_active_slug() === $post->post_name && !$inspection['readiness']['ready']) {
        return new WP_Error(
            'revision_not_ready',
            __('This revision cannot replace the active design because it is incomplete. ', domain: 'novamira')
