...
namespace Novamira\Troubleshoot\ClientIds;
...
function registry(): array
{
    return [
        'claude' => [
            'label' => __('Claude (claude.ai and Claude Desktop connectors)', domain: 'novamira'),
            'client_name' => 'Claude',
            'redirect_uris' => [
...
function generate(string $client_key): array|WP_Error
{
    ...
    if (!array_key_exists($client_key, $registry)) {
        return new WP_Error('unknown_client', __('Unknown AI client.', domain: 'novamira'), ['status' => 400]);
    }
