...
function register(): void
{
    ...
    \WP_CLI::add_command('novamira oauth-keys generate', __NAMESPACE__ . '\\generate_command');
}

/**
 * Create the Novamira OAuth keys if they do not exist yet, then verify they are usable.
 *
 * Reports existence and usability only. No key material and no encryption key is ever printed.
 *
 * ## EXAMPLES
 *
 *     wp novamira oauth-keys generate
 */
function generate_command(): void
{
    ...
    try {
        ...
        \WP_CLI::error(sprintf(
            ...
            __('Novamira could not create its OAuth keys. %s', domain: 'novamira'),
            $e->getMessage(),
        ));
    ...
    if (!keys_are_usable($keys['private']->getKeyContents(), $keys['public']->getKeyContents())) {
        \WP_CLI::error(__(
            'The stored Novamira OAuth private key exists but cannot sign. Delete the "novamira_oauth_private_key" option and run this command again.',
            domain: 'novamira',
        ));
    ...
    \WP_CLI::success(
        $existed
            ? __('Novamira OAuth keys already existed and are usable.', domain: 'novamira')
            : __('Novamira OAuth keys created and verified as usable.', domain: 'novamira'),
    );
}
...
function keys_are_usable(string $private_pem, string $public_pem): bool
{
    ...
    $signature = '';
    if (!openssl_sign('novamira-oauth-key-check', $signature, $private_pem, OPENSSL_ALGO_SHA256)) {
        Keys\clear_openssl_errors();

        return false;
    }

    $verified = openssl_verify('novamira-oauth-key-check', $signature, $public_pem, OPENSSL_ALGO_SHA256);
    Keys\clear_openssl_errors();
